...
export class CharacterSearchContainerComponent {
  ...
  charactersSignal = computed(() => {
    if (this.showRickOnly()) {
      return this.characterSearchService.characters.filter((character) =>
  ...
  constructor(private characterSearchService: CharacterSearchService) {
    ...
    effect(() => {
      console.log(
        ...
        this.characterSearchService.charactersSignal()
      );
    });
  }
  ...
  onNameChange(newName: string): void {
    this.characterSearchService.nameSignal.set(newName);
  }

  onGenderChange(newGender: string): void {
    this.characterSearchService.genderSignal.set(newGender);
  }
